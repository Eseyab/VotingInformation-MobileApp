...
const federal_offices = ['United States Senate', 'United States House of Representatives', 'U.S. Senator', 'U.S. Representative']
...
const selected_county = '';
const selected_local = '';
const all_people = {};
const pseudo_id = 1;
...
function addressSearch() {
    ...
    const show_local   = false;
    const show_county  = false;
    const show_state   = false;
    const show_federal = false;

    const results_level_set = [];
    // set levels from checkboxes
    if (showLocalResult === true) {
        show_local = true;
        results_level_set.push('local');
    }
    if (showCountyResult === true) {
        show_county = true;
        results_level_set.push('county');
    }
    if (showStateResult === true) {
        show_state = true;
        results_level_set.push('state');
    }
    if (showFederalResult === true) {
        show_federal = true;
    ...
    const address = props.searchedAddress;
    ...
    const fetchData = async () => {
        ...
        parseRepData(jsonObject)
        
        //setReps(parseObject)
        //add
    }
}
const parseRepData = (data) => {
    ...
    if (divisions === undefined) {
        ...
        divisions.map(function(division_id, division){
            ...
            if (typeof division.officeIndices !== 'undefined'){
                ...
                $.each(division.officeIndices, function(i, office){
                    const office_name = offices[office];
                    ...
                    $.each(offices[office]['officialIndices'], function(i, official){
                        ...
                        const person = officials[official];
                        info['person'] = person;
        ...
        const template = new EJS({'text': $('#tableGuts').html()});
        ...
        // hook up modal stuff
        const modal_template = new EJS({'text': $('#modalGuts').html()});
        $('.btn-contact').off('click');
        $('.btn-contact').on('click', function(){
            const info = all_people[$(this).data('id')];
...
function findMe() {
    ...
    if (navigator.geolocation) {
        ...
        alert("Geolocation API is not supported in your device.");
    }
};
...
function formatParty(party) {
    if (party) {
        ...
        const party_letter = party.charAt(0);
        const css_class ='label-ind';
        if (party_letter == 'D')
            css_class ='label-dem';
